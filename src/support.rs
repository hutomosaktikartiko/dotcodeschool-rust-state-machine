...
/// This is an `extrinsics`: literally an external message from outside of the blockchain.
...
/// The result type for out runtime. When everything completed successfull, we return `Ok(())`,
...
/// function call.
pub trait Dispatch {
	/// The type used to identity the caller of the function.
	type Caller;
	/// The state transition function call the caller is trying to access.
	type Call;

	/// This fuction takes a `caller` and the `call` the want to make, and returns a `Result`
