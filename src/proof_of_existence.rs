...
pub trait Config: crate::system::Config {
	/// The type which represents the content that can be calimed using this pallet.
...
pub struct Pallet<T: Config> {
	...
	/// Accounts can mmake multiple different claims, but each claim can only have one owner.
	claims: BTreeMap<T::Content, T::AccountId>,
}

impl<T: Config> Pallet<T> {
	/// Crate a new instance of the Proof of Existance Module.
	pub fn new() -> Self {
		Self { claims: BTreeMap::new() }
	}

	/// Gte the owner (if any) of a claim
...
/// We should expect the caller of each call will be provided by the dispatcher,
